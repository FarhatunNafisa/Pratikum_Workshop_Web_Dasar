...
if (!empty($_POST['ubah_password_validate'])) {
    $query = mysqli_query($conn, "SELECT * FROM tb_user WHERE username='" . $_SESSION['user']['email'] . "' AND password='$passwordlama'");
    $hasil = mysqli_fetch_array($query);

    if ($hasil) {
        if ($passwordbaru == $repasswordbaru) {
            $query_update = mysqli_query($conn, "UPDATE tb_user SET password='$passwordbaru' WHERE id='$id'");
            if ($query_update) {
                $message = '<script>alert("Password berhasil diubah"); window.history.back();</script>';
            } else {
                $message = '<script>alert("Password baru gagal diubah"); window.history.back();</script>';
            }
        } else {
